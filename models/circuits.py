...
class Plaquette:
    ...
    def x_rotate(self, q_list):
        for q_ind in q_list:
            self.circuit.h(self.q_register[q_ind])

    def x_back_rotate(self, q_list):
        self.x_rotate(q_list)

    def y_back_rotate(self, q_list):
        for q_ind in q_list:
            self.circuit.sdg(self.q_register[q_ind])
            self.circuit.h(self.q_register[q_ind])

    def y_rotate(self, q_list):
        for q_ind in q_list:
            self.circuit.h(self.q_register[q_ind])
            self.circuit.s(self.q_register[q_ind])
...
class SinglePlaquette(Plaquette):
    ...
    def generate_square_u1(self, q_control, qs_real):
        self.circuit.h(self.q_register[q_control])
